...
class HeaderRowDrawer<T> {
	...
	private float calculateHeaderHeight() {
		...
		if (eventChips == null || eventChips.isEmpty()) {
			return drawConfig.headerTextHeight + drawConfig.headerSecondaryTextHeight + config.headerRowTextSpacing;
		}
		...
		if (containsAllDayEvent) {
			...
			final float eventChipBottomPadding = config.timeColumnTextSize / 4.0f;

			return drawConfig.headerTextHeight + drawConfig.headerSecondaryTextHeight + (headerTextSize
					+ totalEventPadding + eventChipBottomPadding
					+ drawConfig.headerMarginBottom
					+ config.headerRowTextSpacing + headerRowBottomLine);
		} else {
			return drawConfig.headerTextHeight + drawConfig.headerSecondaryTextHeight + config.headerRowTextSpacing + headerRowBottomLine;
		}
	}
	...
	private void drawHeaderRow(Canvas canvas) {
		...
		canvas.clipRect(0, 0, drawConfig.timeColumnWidth, headerHeight);
		canvas.drawRect(0, 0, drawConfig.timeColumnWidth, headerHeight, headerBackground);
