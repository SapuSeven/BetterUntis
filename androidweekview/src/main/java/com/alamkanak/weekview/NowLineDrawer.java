...
class NowLineDrawer {
    ...
    private void drawLine(float startX, float startPixel, Canvas canvas) {
        ...
        // Draw line
        final float minutesUntilNow = now.get(HOUR_OF_DAY) * 60 + now.get(MINUTE) - config.startTime;
        final float lineStartY = startY + minutesUntilNow / 60.0f * config.hourHeight;
